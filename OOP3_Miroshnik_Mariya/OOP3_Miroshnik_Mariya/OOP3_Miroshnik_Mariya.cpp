...
//
//
//// Наполнение файла с записями студентов
...
struct Student {
    ...
    Student(unsigned k, const char* n) : kard(k) {
...
struct IndexEntry {
    ...
    // Создание индексных файлов
    static void createIndexes(const char* dataFilename, const char* idx1Filename, const char* idx2Filename) {
        FILE* dataFile;
        errno_t err = fopen_s(&dataFile, dataFilename, "rb");
        if (err != 0 || !dataFile) {
            perror("Ошибка открытия файла данных!");
        ...
        //// Получаем количество записей
        //fseek(dataFile, 0L, SEEK_END);
        //long numRecords = ftell(dataFile) / sizeof(Student);
        //rewind(dataFile);
    ...
    static void findByCardNumber(const char* dataFilename, const char* idxFilename, unsigned cardNum) {
        ...
        if (found != NULL) {
            // Переходим к нужной записи в файле данных
            fseek(dataFile, found->index * sizeof(Student), SEEK_SET);
            Student s;
            fread(&s, sizeof(Student), 1, dataFile);
            s.printStudent();
        }
        else {
            printf("Запись с номером зачетки %u не найдена!\n", cardNum);
    ...
    static void listNamesSortedByLength(const char* dataFilename, const char* idxFilename) {
        ...
        // Проход по каждому индексу и получение записей
        for (long i = 0; i < numRecords; ++i) {
            fseek(dataFile, indexes[i].index * sizeof(Student), SEEK_SET);
            Student s;
            fread(&s, sizeof(Student), 1, dataFile);
            printf("Длина: %2zu | Фамилия: %-20s | Зачетка: %u\n", indexes[i].len, s.name, s.kard);
        }
